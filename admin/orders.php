...
// Apply search filter
if (!empty($search)) {
    $orderDetails = array_filter($orderDetails, function($ord) use ($search) {
        $searchLower = strtolower($search);
        return strpos(strtolower($ord['order_number'] ?? ''), $searchLower) !== false ||
               strpos(strtolower($ord['user']['first_name'] ?? ''), $searchLower) !== false ||
               strpos(strtolower($ord['user']['last_name'] ?? ''), $searchLower) !== false ||
               strpos(strtolower($ord['user']['email'] ?? ''), $searchLower) !== false;
    });
}

// Apply status filter
if (!empty($statusFilter)) {
    $orderDetails = array_filter($orderDetails, function($ord) use ($statusFilter) {
...
// Apply date filter
if (!empty($dateFilter)) {
    $orderDetails = array_filter($orderDetails, function($ord) use ($dateFilter) {
        ...
        <aside class="w-64 bg-white shadow-lg min-h-screen">
            <nav class="mt-8">
                <div class="px-4 space-y-2">
                    ...
                    <a href="orders.php" class="flex items-center px-4 py-2 text-primary bg-blue-50 rounded-lg relative">
        ...
        <main class="flex-1 p-8">
                ...
                <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
                    <div class="flex-1">
                        <form method="GET" class="flex gap-2">
                            ...
                            <?php if (!empty($_GET['search']) || !empty($_GET['status_filter']) || !empty($_GET['date_filter'])): ?>
                            <a href="orders.php" class="bg-gray-500 text-white px-6 py-2 rounded-md hover:bg-gray-600 transition-colors">
                    ...
                    <div class="flex gap-2">
                        <select name="status_filter" onchange="this.form.submit()" class="px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-primary focus:border-primary">
                            ...
                            <option value="shipped" <?php echo ($_GET['status_filter'] ?? '') === 'shipped' ? 'selected' : ''; ?>>Shipped</option>
                        ...
                        <input type="date" name="date_filter" value="<?php echo htmlspecialchars($_GET['date_filter'] ?? ''); ?>" 
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h2 class="text-lg font-semibold text-gray-900">All Orders (<?php echo count($orderDetails); ?> found)</h2>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">Rs. <?php echo number_format($ord['total_amount'], 2); ?></div>
